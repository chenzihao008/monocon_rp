...
class BaseEngine:
    def __init__(self, 
        ...
        if cfg['TRAIN_TYPE']=='DDP' and not is_test:
            self.train_dataset, self.train_loader,self.sampler = \
                self.build_loader(is_train=True) if not is_test else (None, None)
            self.test_dataset, self.test_loader,self.sampler = self.build_loader(is_train=False)
        else:
            self.train_dataset, self.train_loader,self.sampler = \
                self.build_loader(is_train=True) if not is_test else (None, None)
            self.test_dataset, self.test_loader,self.sampler = self.build_loader(is_train=False)
    ...
    # def train_DDP(local_rank=None,arg=None) -> None:
    #     self = arg.self
    ...
    #     train_sample = DistributedSampler(self.train_dataset)
    #     train_loader = DataLoader(self.train_dataset,batch_size=self.train_loader.batch_size,sampler=train_sample)
    #     model = self.model 
    #     model.to(local_rank)
    ...
    #         # Validation
    #         if local_rank == 0 and (self.val_period > 0) and (epoch % self.val_period == 0):
    #             model.eval()
    ...
    def load_checkpoint(self, 
        ...
        for attr_k, attr_v in attrs.items():
            if  attr_k not in ['target_epochs','val_period']  :
