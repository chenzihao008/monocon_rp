...
class MonoconEngine_ddp(BaseEngine):
    def __init__(self, cfg: CfgNode,local_rank, **kwargs):    
        self.local_rank = local_rank
        self.is_ddp = cfg['TRAIN_TYPE']
        super().__init__(cfg, **kwargs)
    ...
    def build_model(self):
        detector = MonoConDetector(
            num_dla_layers=self.cfg.MODEL.BACKBONE.NUM_LAYERS,
            pretrained_backbone=self.cfg.MODEL.BACKBONE.IMAGENET_PRETRAINED).to(self.local_rank)
        # mem_reporter = MemReporter(detector)
        # print(mem_reporter.report())
        detector = DDP(detector,device_ids=[self.local_rank],output_device=self.local_rank,find_unused_parameters=True)
        return detector
    ...
    def train_one_epoch(self) -> float:
        ...
        for batch_idx, data_dict in enumerate(self.train_loader):
            ...
            # Forward
            data_dict = move_data_device(data_dict, self.local_rank)
            _, loss_dict = self.model(data_dict)
            total_loss = reduce_loss_dict(loss_dict)
            total_loss.backward()
